...
class TransactionImportService {
    ...
    async _searchBankEmails(daysBack, maxEmails) {
        try {
            // Query para buscar emails de bancos
            const query = 'from:@bbva.com OR from:@mercadopago.com OR from:@nu.com OR from:@plata.com.mx newer_than:' + daysBack + 'd';
    ...
    async _processEmails(emailIds, skipDuplicates) {
        ...
        for (const emailInfo of emailIds) {
            try {
                processed++;
                this.reportProgress(`Procesando email ${processed}/${emailIds.length}...`);
    ...
    async _checkExistingTransaction(emailId) {
        ...
        // En la práctica, consultaríamos la BD
        // Por ahora retornamos null (no verificar duplicados)
        return null;
    }
